...
const SignIn = () => {
	...
	return (
		<div className="fixed left-0 right-0 bottom-0 top-0 z-50 bg-juejin-mask-1 ">
			<form
				onSubmit={onSubmit}
				className="layer absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[--login-box-width] bg-juejin-layer-1 rounded-lg py-[--login-box-padding]"
			>
				...
				<div className="p-[--login-box-padding]">
					...
					<div className="flex flex-col mt-4">
						...
						/>
						<small
							className={`opacity-0 text-juejin-danger-1-normal ${errors.email ? " opacity-100" : ""}`}
						>
						...
						/>
						<small
							className={` opacity-0 text-juejin-danger-1-normal ${errors.password ? " opacity-100" : ""}`}
						>
							密码不能为空
						</small>
					</div>
					<button className="btn-primary w-full h-10">登录</button>
					<div className="mt-7 leading-8 h-8">
						<span>其他登录：</span>
						<AiFillGithub
							onClick={signInGithub}
							className=" inline ml-4 text-[2rem] -translate-y-[2px] cursor-pointer hover:text-juejin-brand-2-hover"
							title="icon_Github"
						/>
					</div>
					<p className="text-center mt-7">
						注册登录即表示同意
						<Link href="/" className="text-juejin-brand-1-normal">
							&nbsp;用户协议
						</Link>
						&nbsp;和&nbsp;
						<Link href="/" className="text-juejin-brand-1-normal">
