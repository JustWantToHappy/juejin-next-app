...
const ArticleLayout: React.FC<React.PropsWithChildren & { articleId: string, likeCount: number }> = (props) => {
  ...
  const giveAnLike = async () => {
    ...
    if (session.user.id) {
      Patch(`/api/like`, { userId: session.user.id, articleId: props.articleId }).then((res) => {
  ...
  const icons: { icon: IconType, href?: string, onClick?: MouseEventHandler }[] = [
  ...
  React.useLayoutEffect(() => {
    (async () => {
      const data = await Get<string>(`/api/like?userId=${session?.user.id}&articleId=${props.articleId}`)
      ...
      <ul className='hidden sm:flex flex-col fixed z-30 left-3 top-[140px] gap-y-6 w-14 h-[400px]  '>
        {icons.map((item, index) => {
          ...
          return (<li
            ...
            <Icon size='20' className={`  ${index === 0 && isLike ? ' text-juejin-brand-1-normal' : 'group-hover:text-juejin-font-2'}`} />
          </li>)
        })}
      </ul>
    </>
