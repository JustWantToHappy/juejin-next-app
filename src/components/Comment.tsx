...
const Comment: React.FC<{ articleId: string }> = (props) => {
	...
	const currentTotal = React.useMemo(() => {
		return (
			data?.reduce(
				(currentValue, { listLength }) => currentValue + listLength,
				0,
			) || 0
		);
	}, [data]);
	...
	return (
		<div data-id="comment" ref={commentRef}>
				...
				<div className="flex-1">
					{session ? (
						...
						<>
							...
							>
								看完啦，
								<button className="mx-1 text-juejin-brand-1-normal cursor-pointer">
									登录
								</button>
								分享一下感受吧~
							</div>
			...
			<div className="mt-8">
				{data?.map(({ data: comments }) => {
					return comments.map((firstLevelComment) => (
						<div key={firstLevelComment.id} className="mb-8">
							<CommentItem
							...
							>
								{firstLevelComment.children.length ? (
									<div className="my-3 layer bg-juejin-layer-2-1 p-4 flex flex-col gap-y-3">
										{firstLevelComment?.children?.map(
											(secondLevelComment) => (
												<CommentItem
													...
													avatarSize={30}
													createdAt={
														secondLevelComment.createdAt
													}
													updateComments={
														updateComments
													}
													parent={
														secondLevelComment.parent
													}
													updateActiveCommentInput={
														updateActiveCommentInput
													}
													...
													articleId={articleId}
													user={
														secondLevelComment.user
													}
													key={secondLevelComment.id}
													likes={
														secondLevelComment.likes
													}
													content={
			...
			<p
				...
				style={{
					display: `${currentTotal >= total ? "flex" : "none"}`,
				}}
			>
				已经到底了(๑•̀ω•́)ノ
			</p>
