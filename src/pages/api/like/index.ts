	...
	if (req.method === Method.PATCH) {
		...
		if (like) {
			...
			await prisma.$transaction([
				...
				prisma.article.update({ where: { id: req.body.articleId }, data: { likeCount: { decrement: 1 } } })
			])
			res.status(200).json('')
		} else {
			await prisma.$transaction([
				prisma.like.create({ data: req.body }),
				prisma.article.update({ where: { id: req.body.articleId }, data: { likeCount: { increment: 1 } } })
			])
