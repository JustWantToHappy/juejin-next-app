...
export default async function handler(
	...
	if (req.method === Method.GET) {
		...
		const comments: Array<ResCommentType> = firstLevelComments.map(
			(firstLevelComment) => ({ ...firstLevelComment, children: [] }),
		);
		//评论总数
		const total = await prisma.comment.count({ where: { articleId } });
		await Promise.all(
			firstLevelComments.map(async (firstLevelComment) => {
				const rootId = firstLevelComment.id;
				const comment = comments.find(
					(comment) => comment.id === firstLevelComment.id,
				);
				...
				secondLevelComments?.map((secondLevelComment, _, arrThis) => {
					const parent = arrThis.find(
						(comment) => comment.id === secondLevelComment.parentId,
					);
					//直接回复一级评论的二级评论
		...
		const listLength =
			...
			comments.reduce(
				(currentValue, comment) =>
					currentValue + comment.children.length,
				0,
			);
